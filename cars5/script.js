...
img.src = "images/wall_skater3.jpg"
...
function hidePicture(obj,speed,options){

		var timeline = new TimelineMax();
		timeline.to(obj,0,{ visible: false, ease: Linear.easeNone})
    	console.log("Show Picture Fired")
        return timeline;
}
...
function play(){
	...
	var tempMax = Math.max($distance1.val(),$distance2.val())
	...
	if ( tempMax > (800 * 0.9) ) {
		//coll.push(showPicture(shape2))
		   ...
		   if (vehicle3.visible==false){
		  		 ...
		  		 tl.add(showPicture(vehicle3),"a")
			   
		   }
		   
		   if (vehicle4.visible==false){
		       
		   		tl.add(showPicture(vehicle4),"a")
...
function init() {
	...
	shape.graphics.beginBitmapFill(img);
	shape.graphics.drawRect(0,0,840,400);
 ...
 //];
	...
	shape2.graphics.drawRect(0,0,840,400);
...
function buildTimeline() {
	...
	function updateUI2() {
		...
		$distance1.val(tempDist1);
		$distance2.val(tempDist2);
		//console.log(vehicle1.x);
